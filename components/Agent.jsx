...
const Agent = ({ userName, userId, avatar, classroomId }) => {
  ...
  useEffect(() => {
    console.log("Component mounted, calling listModels...");
    listModels();
  }, []);
  ...
  const handleUpload = useCallback(async (file) => {
    ...
    try {
      ...
      setError(
        error.message || "Failed to process your document. Please try again."
      );
    } finally {
    ...
    <>
        ...
        <div className="card-classroomer m-4">
          <div className="avatar">
            <Image
              ...
              width={50}
              height={50}
              className="object-cover size-[50px] sm:size-[65px]"
            />
      ...
      {error && (
        <div className="my-4 max-w-xl mx-auto">
          <p className="text-center text-destructive-100 text-sm sm:text-base">
            {error}
          </p>
        </div>
      )}
        ...
        <div className="transcript-border my-4 max-w-3xl mx-auto">
          <div className="transcript m-1">
            <p
              key={lastMessage}
              className={cn(
                "transition-opacity duration-500 opacity-0 text-sm sm:text-base",
